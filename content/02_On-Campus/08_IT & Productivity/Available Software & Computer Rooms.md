---
title: Available Software & Computer Rooms
section: 02_On-Campus/08_IT & Productivity
tags: [software, computer-rooms, IT, campus, licensing]
status: ready
last_updated: 2026-07-30
---

What's on the public PCs, what you can install on your own laptop for free, and how to reserve a room.

---

## Computer Rooms

Two rooms on MLIC 1F, open to all students/faculty/staff with an IUJ network account. When a room is reserved for a course, use the other one.

| | PC120 | PC121 |
|---|---|---|
| Computers | 73 | 27 |
| OS | Windows 11 | Windows 11 |
| Printers | 1 color (`stcolor`), 2 B&W (`stprt5`/`stprt6`) | 1 B&W (`stprt7`) |
| Scanners | — | 2 |
| Adobe CC terminal | — | 1 |
| Database terminals | — | Eikon, NVivo, SPSS |
| Projectors | 2 | 2 |

*(MLIC's official IT Guide lists 73/27; IUJ's live room-reservation page showed 72/25 as of mid-2026 — likely reflects units temporarily down rather than a capacity change.)*

Smaller dorm PC lounges also exist (SD2 5F, SD3 5F: 4 computers each, 1 color printer each) but have no dedicated software beyond Office/STATA.

> ⚠️ **All public computers reset on restart** — your personal files will be gone. Don't save work locally; use your IUJ Google Drive or a USB drive.

**Reserving a room**: courses and TA sessions get priority and are scheduled in advance; IUJ offices reserve rooms via an online form. If a room shows as reserved, use the other one — check the posted schedule at the room or ask IT Helpdesk.

---

## Software on Public PCs

**Regular** (both rooms): Microsoft Office 2021, STATA

**Special** (PC120/PC121, varies by room — see table above): E-Views, Stat Transfer, EIKON / LSEG Workspace, SPSS, NVivo, Adobe Creative Cloud (Acrobat, Photoshop, Premiere Pro, Illustrator, etc.)

Some special software (EIKON/LSEG, SPSS, NVivo in PC121) is limited to a **single terminal** — expect to queue during busy periods (thesis season, econometrics coursework).

---

## Installing Free Licensed Software on Your Own PC

Regular students (not non-regular/exchange) can install the following free of charge:

| Software | Notes |
|---|---|
| Microsoft Office Suite | Full 2021 Professional Plus (Word, Excel, PowerPoint, Access, Publisher, InfoPath, OneNote) |
| STATA | Activation required at MLIC office |
| Stat Transfer | Trial only on personal PCs |
| Windows 10/11 (Mac only) | ¥3,000 fee; free for Mac — requires Intel Mac; Parallels Desktop recommended to run it |

Apply at **[iuj.ac.jp/cs/application_form](https://www.iuj.ac.jp/cs/application_form/)**. You'll need to register your personal info on that page first. E-Views, EIKON/LSEG, SPSS, and NVivo are **not** installable on personal computers — PC room use only.

---

## Services to Be Charged

| Service | Fee |
|---|---|
| OS recovery | ¥3,000 |
| Data rescue | ¥5,000 |
| A4 photo paper (for color laser printer, MLIC) | ¥15/sheet |

Ask MLIC office for exact terms before committing.

---

## Related Articles
- [[Windows 11 Setup Guide]]
- [[Printing & Scanning — Campus & Conbini]]
- [[IUJ Network & Email Accounts]]

---

## 🗣️ Senior Submissions
> *Have a tip, correction, or experience to add? Contact [your name/handle].*

- Typical wait times for single-terminal software (EIKON, SPSS, NVivo) during peak season
- Whether the free Office/STATA install process was smooth for you
