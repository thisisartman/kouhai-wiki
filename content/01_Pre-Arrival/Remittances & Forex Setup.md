---
title: Remittances & Forex Setup
section: 01_Pre-Arrival
tags: [pre-arrival, finance, remittance, forex, wise, money]
status: ready
last_updated: 2026-05-28
---

# 💸 Remittances & Forex Setup

Setting up your money systems before you leave is one of the highest-ROI things you can do pre-arrival. A bad remittance setup costs you real money every month. A good one is nearly invisible.

This article covers what to set up before you leave. For ongoing banking in Japan, see [[JP Bank vs Daishi — Comparison & How to Open]].

---

## The Core Problem

Japan is still heavily cash-based in rural areas. You need yen on arrival, and you'll need to move money between your home country and Japan throughout your stay. The default options (bank wire transfers, airport forex) are expensive. The smart options require setup before you leave.

---

## What to Set Up Before Departure

### 1. Wise (formerly TransferWise) — Recommended Primary Tool

Wise is the best general-purpose tool for international students in Japan. It lets you hold multiple currencies, convert at near-interbank rates (small fixed fee), and send/receive internationally.

**Set up before leaving because:**
- Verification requires uploading identity documents — easier on a stable internet connection at home
- Some countries require additional verification steps that take days

**What to do:**
1. Create account at [wise.com](https://wise.com)
2. Verify your identity (passport + selfie)
3. Add your home bank account as a source
4. Get the Wise card if available in your country (physical debit card — works in Japan's ATMs and most card terminals)

**Use case at IUJ:**
- Receive money from family in home currency → convert to JPY → spend or withdraw
- Send money home (cheaper than bank wire)
- Hold JPY balance and use the Wise card for purchases

> 💡 Wise's JPY→INR or INR→JPY rates are significantly better than any bank wire or forex bureau.

---

### 2. Forex Card / Travel Card (Backup)

If Wise isn't available or set up in time, a multi-currency forex card from your home bank is a reasonable backup.

**Indian students:** HDFC Multi-Currency Forex Card, ICICI Travel Card, or Niyo Global card are popular options. Load them with JPY before departure.

**Pros:** Locked-in exchange rate, widely accepted at Japanese ATMs and shops
**Cons:** Typically higher fees than Wise; loading rates vary

---

### 3. Home Bank International Access

Check before you leave that your home bank card (debit or credit) works internationally. Specifically:

- Is it on a Visa/Mastercard network? (Not just RuPay or domestic networks)
- Does it work for international withdrawals? Enable this in your bank app.
- What's the per-transaction international fee? (Can be 2–4%+ per withdrawal)

Japan Post Bank (JP Bank) ATMs accept most Visa/Mastercard international cards — useful in emergencies. 7-Eleven ATMs (Japan Post/Seven Bank) also accept international cards and have English interfaces.

---

### 4. Cash for Arrival

Bring at least **¥20,000–30,000** in cash for the first few days. You'll need it for:
- Bus/taxi from the station before you have an IC card
- Convenience store purchases
- Any immediate needs before your JP Bank account is open

**Where to get yen at home:**
- Airport forex on your way out (not ideal rates, but convenient)
- Your home bank's currency service (order in advance — better rates than airport)
- Wise transfer to a yen-holding card

> ⚠️ Do NOT rely on exchanging money at Narita Airport on arrival. The rates are poor and you'll be tired. Bring cash from home.

---

## What NOT to Use

| Method | Why to Avoid |
|---|---|
| Bank wire (SWIFT) | Fees of ¥2,000–5,000 per transfer + poor rates |
| Airport forex bureaus | Worst exchange rates you'll encounter |
| Western Union / MoneyGram | Expensive; designed for unbanked recipients |
| Credit card cash advances | High interest from day one; avoid entirely |

---

## Remittance: Sending Money Home

Once in Japan, the main options for sending money home:

| Service | Best For | Notes |
|---|---|---|
| **Wise** | Most countries; best rates | Set up before arrival |
| **Remitly** | India, Philippines, South/SE Asia | Good rates; mobile app-first |
| **Japan Post (国際送金)** | Anywhere | Available at post offices; more expensive but no tech barriers |
| **JP Bank international transfer** | Direct bank-to-bank | Requires recipient bank details; reasonable fees |

> 💡 Wise or Remitly will beat JP Bank rates for most India-Japan transfers. Set them up before you need them — identity verification takes time.

For full details on ongoing remittances, see [[Sending Money Home]].

---

## Forex Prep Checklist

- [ ] Create and verify Wise account (do this 2+ weeks before departure)
- [ ] Order/enable Wise card if available
- [ ] Check home debit/credit card works internationally and has network access
- [ ] Withdraw ¥20,000–30,000 in cash before departure (or at home bank)
- [ ] Download Wise app and Remitly app on phone
- [ ] Note your home bank's international wire fee and SWIFT code (for future reference)

---

## Related Articles
- [[JP Bank vs Daishi — Comparison & How to Open]]
- [[Sending Money Home]]
- [[PayPay — Setup, Linking Banks, Loading Money]]
- [[Student Discounts]]

---

## 🗣️ Senior Submissions
> *Have a tip, correction, or experience to add? Contact [your name/handle].*

- Which remittance services worked best for your home country
- Wise setup tips or gotchas specific to certain countries
- Best forex cards for Indian students
- How much cash you actually needed in the first week
