...
**Local build fails with `Could not resolve "./status-badge"` (or `home-search`,
`close-nav-on-outside-tap`, `suggest-edit`):** `.quartz/plugins/*` are symlinks
back to the real plugin source in `quartz-plugins/`, generated by
`install-plugins` — they are not tracked in git (`.quartz/` is gitignored), so
they're baked with an absolute path at generation time. If this repo's checkout
ever gets moved or renamed (it happened once: `Projects/Kouhai Wiki/` →
`Projects/IUJ/Kouhai Wiki/`), the symlinks keep pointing at the old absolute
path and silently break. `npx quartz build` fails with unresolved-import
errors; re-running `npm run install-plugins` doesn't fix it either — it errors
on the same `.scss`-import issue described above before it gets to relinking.
Fix: relink by hand, pointing each symlink at the current absolute path —

```bash
for p in close-nav-on-outside-tap home-search status-badge suggest-edit; do
  rm ".quartz/plugins/$p"
  ln -s "$(pwd)/quartz-plugins/$p" ".quartz/plugins/$p"
done
```

Confirmed 2026-08-27: `npx quartz build` builds clean (113 files, no
warnings) after relinking.
